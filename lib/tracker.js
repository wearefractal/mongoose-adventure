...
if (!fs.existsSync(trackingFolder)) fs.mkdirSync(trackingFolder);
...
us.get = function(name) {
  var logPath = us.logFile(name);
  if (fs.existsSync(logPath)){
    return ton.parse(fs.readFileSync(logPath));
  }
  return null;
};
...
us.wipe = function(name) {
  ...
  var logPath = us.logFile(name);
  if (fs.existsSync(logPath)) rimraf(logPath);
  return us;
};
